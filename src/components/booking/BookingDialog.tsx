...
import { appFetch } from "../../../mobile/backend";
import { openExternal } from "../../../mobile/runtime";
import { checkoutURL } from "../../../mobile/policy";
...
export function BookingDialog({
  ...
  async function handleFreeBooking() {
    ...
    try {
      ...
      const res = await appFetch("/api/bookings/free", {
        method: "POST",
        headers: {
        ...
        {service && done && (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Your session with {service.providerName} is confirmed. You can message your provider or add it to your calendar below.
              </DialogDescription>
            </DialogHeader>
          ...
          <>
            ...
            {service.price > 0 ? (
              <div className="space-y-3">
                <p className="text-sm text-muted-foreground">Complete paid bookings securely on LifeSwap in the browser. Sign in there with the same account, then return here to see your session and messages.</p>
                <Button className="w-full" onClick={async()=>{try{await openExternal(checkoutURL(service.id));}catch{setError('Could not open checkout. Please try again.');}}}>Continue to secure checkout</Button>
                <p className="text-xs text-muted-foreground">You’ll choose the date and time again on the checkout page.</p>
              </div>
            ) : (
              <>
                <Button
